...
- Initial implementation intentionally uses synchronous REST calls to simplify debugging and establish API correctness first.
- Message timelines currently rely on `GET /channels/{channel.id}/messages`; Gateway streaming is planned next.
- Rate-limit handling currently surfaces 429 with retry metadata to the user; full bucket scheduler is planned next.

## Next Milestones

1. Add Gateway connection lifecycle (`/gateway`, Hello/Heartbeat/Identify/Resume).
2. Incremental room updates from `MESSAGE_CREATE|UPDATE|DELETE` dispatch events.
