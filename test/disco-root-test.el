...
(ert-deftest disco-root-activity-candidates-exclude-unresolved-and-hidden-channels ()
  (let ((disco-root-activity-include-threads nil))
    (cl-letf (((symbol-function 'disco-state-private-channels) #'ignore)
              ...
              ((symbol-function 'disco-state-guild-channels)
               (lambda (_guild-id)
                 '(((id . "unknown") (guild_id . "g1") (type . 0))
                   ...
                   ((id . "visible") (guild_id . "g1") (type . 0)
                    (permissions . "1024")))))
              ((symbol-function 'disco-state-guild-threads) #'ignore))
      (should
       (equal '("visible")
...
(ert-deftest disco-root-visible-guild-unread-total-excludes-inaccessible-channels ()
  (let ((disco-root--view-mode 'all)
        counted)
    (cl-letf (((symbol-function 'disco-state-guild-channels)
               (lambda (_guild-id)
                 '(((id . "unknown") (guild_id . "g1") (type . 0))
                   ((id . "hidden") (guild_id . "g1") (type . 0)
                    (permissions . "0"))
...
(ert-deftest disco-root-state-reset-invalidates-header-cache ()
  (with-temp-buffer
    (disco-root-mode)
    (setq disco-root--header-state-cache '(:filters "stale")
          disco-root--last-header-refresh-at 10.0)
    (cl-letf (((symbol-function 'force-mode-line-update) #'ignore))
      (disco-root--handle-state-reset))
    (should-not disco-root--header-state-cache)
    (should-not disco-root--last-header-refresh-at)))

(ert-deftest disco-root-attach-preserves-deferred-header-refresh ()
  (with-temp-buffer
    (disco-root-mode)
    (setq disco-root--header-refresh-timer 'pending)
    (let ((cancelled nil))
      (cl-letf (((symbol-function 'disco-root--cancel-live-update-timer) #'ignore)
                ((symbol-function 'disco-root--cancel-header-refresh-timer)
...
(ert-deftest disco-root-maybe-refresh-activity-header-line-defers-until-deadline ()
  (with-temp-buffer
    (disco-root-mode)
