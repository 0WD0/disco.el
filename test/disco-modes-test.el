...
(ert-deftest disco-client-mode-line-counts-only-viewable-channels ()
  (cl-letf (((symbol-function 'disco-state-channels)
             (lambda () '(((id . visible)) ((id . hidden)))))
            ((symbol-function 'disco-state-channel-viewable-p)
             (lambda (channel _unknown-value)
               (eq (alist-get 'id channel) 'visible)))
            ((symbol-function 'disco-state-channel-unread-mention-count)
             (lambda (_channel-id) 2))
            ((symbol-function 'disco-state-channel-muted-p) #'ignore)
            ((symbol-function 'disco-state-channel-known-unread-message-count)
             (lambda (_channel) 3)))
    (should (equal '(3 . 2) (disco-client-mode-line--counts)))))
...
(ert-deftest disco-client-mode-line-update-skips-unrelated-gateway-events ()
  (let ((disco-client-mode-line-mode t)
    ...
    (cl-letf (((symbol-function 'disco-client-mode-line--counts)
      ...
      (should (= 1 scans))
      (should (equal '(4 . 5) disco-client-mode-line--cached-counts))
      (disco-client-mode-line-update '(:type channel-sync))
      (should (= 2 scans)))))
