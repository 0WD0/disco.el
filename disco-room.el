...
(defvar-local disco-room--displayed-message-ids nil)
...
(defun disco-room--search-match-message-p (msg filter)
...
(defun disco-room--message-line-positions ()
  "Return ordered list of rendered message line positions in current room buffer."
  (let (positions)
    (save-excursion
      (goto-char (point-min))
               ...
               (lambda (pos)
                 (and (> pos cursor)
                      (let* ((message-id (get-text-property pos 'disco-message-id))
                             (msg (and message-id (disco-room--message-by-id message-id))))
                        (and msg (disco-room--search-match-message-p msg filter)))))
               (disco-room--message-line-positions)))
      (dolist (pos (disco-room--message-line-positions))
        (when (and (< pos cursor)
                   (let* ((message-id (get-text-property pos 'disco-message-id))
                          (msg (and message-id (disco-room--message-by-id message-id))))
                     (and msg (disco-room--search-match-message-p msg filter))))
...
(defun disco-room--inplace-search-move-local (filter forward &optional count)
  ...
  (let ((steps (max 1 (or count 1)))
    ...
    (while (and moved (> steps 0))
      (let ((hit (or (disco-room--search-message-line filter forward cursor)
                     (disco-room--search-message-line filter forward
  ...
  (let ((seen (make-hash-table :test #'equal))
    ...
    (when-let* ((current-user-id (and (fboundp 'disco-gateway-current-user-id)
      ...
      (dolist (label (list "me" current-user-id))
        (setq result (disco-room--search--register-candidate label current-user-id seen result))))
        ...
        (let* ((user (alist-get 'user member))
          ...
          (when user-id
            (dolist (label (delq nil (list nick
                                           ...
                                           user-id)))
              (setq result (disco-room--search--register-candidate label user-id seen result))))))
      ...
      (let ((author-id (disco-room--message-author-id msg)))
        (when author-id
          (dolist (label (delq nil (list (disco-room--message-author msg)
                                         author-id)))
            (setq result (disco-room--search--register-candidate label author-id seen result))))))
    (nreverse result)))
...
(defun disco-room--read-search-user-id (prompt)
  ...
  (let* ((candidates (disco-room--search-user-candidates))
         (default-id (and (ignore-errors (disco-room--message-id-at-point))
                          (ignore-errors
                            (disco-room--message-author-id (disco-room--message-at-point)))))
         (default-label (and default-id
                             (car (seq-find (lambda (cell)
                                              (equal (cdr cell) default-id))
                                            candidates))))
         (choice (completing-read prompt (mapcar #'car candidates) nil t nil nil default-label)))
...
(defun disco-room--msg-filter-status-line ()
  ...
  (when (disco-room--msg-filter-active-p)
    (let* ((filter disco-room--msg-filter)
           ...
           (parts (list (format "Filter: %s" (disco-room--msg-filter-title filter))
                        (if (numberp total)
...
(defun disco-room--filter-callback-active-p (room-buffer channel-id generation)
...
(defun disco-room--inplace-search-callback-active-p (room-buffer channel-id generation)
...
(defun disco-room--run-msg-filter (filter &optional append)
    ...
    (disco-room--search-current-channel-async
     ...
     (lambda (body)
       (when (disco-room--filter-callback-active-p room-buffer channel-id generation)
         (with-current-buffer room-buffer
           (let* ((page (disco-room--search-flatten-messages (alist-get 'messages body)))
                  (items (if append
             ...
             (setq-local disco-room--msg-filter
                         (list :active t
                               ...
                               :has-more (if (numberp total)
                                             (< (length items) total)
                                           (= (length page) disco-room-filter-search-limit))))
             (if (disco-room--at-message-bottom-p)
                 (disco-room-render)
               (disco-room--render-preserving-point))
             (message "disco: filter -> %s" (disco-room--msg-filter-title disco-room--msg-filter))))))
     :on-error
     (lambda (err)
       (when (disco-room--filter-callback-active-p room-buffer channel-id generation)
         (with-current-buffer room-buffer
...
With BY-SENDER-P, also prompt for a sender and restrict matches to that user."
  ...
  (let ((sender-id (when by-sender-p
    ...
    (disco-room--run-msg-filter (list :query (unless (string-empty-p trimmed) trimmed)
                                      :author-id sender-id)
                                nil)))
...
(defun disco-room-filter-load-more ()
  ...
  (cond
   ...
   (t
    (disco-room--run-msg-filter disco-room--msg-filter t))))
...
(defconst disco-room--inplace-search-filters
...
(defun disco-room--inplace-search-title (filter)
...
(defun disco-room--inplace-search-current-message-id (forward)
...
(defun disco-room-inplace-search ()
  ...
  (let* ((choice (completing-read "Room search: "
                                  (mapcar #'car disco-room--inplace-search-filters)
                                  nil t)))
    (call-interactively (or (cdr (assoc choice disco-room--inplace-search-filters))
...
When FORWARD-P is non-nil, continue toward newer messages."
  ...
  (if disco-room--inplace-search-filter
      (disco-room--inplace-search-dispatch disco-room--inplace-search-filter forward-p)
...
(defun disco-room--ensure-ewoc (&optional channel draft)
  ...
  (unless disco-room--ewoc
    (let ((disco-room--rendering t)
          ...
          (buffer-undo-list t))
      (erase-buffer)
      (setq disco-room--displayed-message-ids nil)
...
(defun disco-room--message-ids-in-order (messages)
  "Return MESSAGE ids in render order, skipping missing ids."
  (delq nil
        (mapcar (lambda (msg)
                  (and (listp msg) (alist-get 'id msg)))
                (or messages '()))))

(defun disco-room--timeline-order-compatible-p (current-ids target-ids)
  "Return non-nil when CURRENT-IDS can be reconciled into TARGET-IDS in place."
  (let ((current-set (make-hash-table :test #'equal))
        common-current
        common-target)
    (dolist (message-id current-ids)
      (puthash message-id t current-set))
    (dolist (message-id current-ids)
      (when (member message-id target-ids)
        (push message-id common-current)))
    (dolist (message-id target-ids)
      (when (gethash message-id current-set)
        (push message-id common-target)))
    (equal (nreverse common-current)
           (nreverse common-target))))

(defun disco-room--insert-message-node-before (msg before-node)
  "Insert one message node for MSG before BEFORE-NODE, or at end when nil."
  (when disco-room--ewoc
    (let ((disco-room--rendering t)
          (inhibit-read-only t)
          (buffer-undo-list t))
      (let* ((node (if before-node
                       (ewoc-enter-before disco-room--ewoc before-node msg)
                     (ewoc-enter-last disco-room--ewoc msg)))
...
(defun disco-room--reconcile-timeline (ordered-messages)
  ...
  (disco-room--ensure-ewoc)
  (let* ((target-ids (disco-room--message-ids-in-order ordered-messages))
         (current-ids (or disco-room--displayed-message-ids '()))
         (target-set (make-hash-table :test #'equal)))
    (dolist (message-id target-ids)
      (puthash message-id t target-set))
    (unless (disco-room--timeline-order-compatible-p current-ids target-ids)
      (disco-room--clear-timeline)
      (setq current-ids '()))
    (dolist (message-id current-ids)
      (unless (gethash message-id target-set)
        (disco-room--delete-message-node message-id)))
    (cl-loop for msg in ordered-messages
             for index from 0 do
             (let* ((message-id (alist-get 'id msg))
                    (node (and message-id
               ...
               (if node
                 ...
                 (let ((before-node
                        (cl-loop for later-id in (nthcdr (1+ index) target-ids)
                                 for later-node = (gethash later-id disco-room--message-node-table)
                                 when later-node return later-node)))
                   (disco-room--insert-message-node-before msg before-node)))))
    (setq disco-room--displayed-message-ids target-ids)
    (disco-room--rebuild-render-contexts ordered-messages)
    (let ((disco-room--rendering t)
          (inhibit-read-only t)
          (buffer-undo-list t))
...
(defun disco-room--insert-message-node (msg)
  ...
  (disco-room--insert-message-node-before msg nil))
